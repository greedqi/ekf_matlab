...
hold on
plot(p_data(1:300,1),p_data(1:300,2),'-og',sensor_data(1:301,1),sensor_data(1:301,2),'-or',ekf_local_data(1:301,1),ekf_local_data(1:301,2),'-ok')


%计算模型预测与测量值的协方差，理论上predict_senor_covariance=Q，P，R协方差的叠加，与ekf无关debug用
predict_senor_covariance=zeros(5,5);
for i=1:ekf_rows-1
    for j=1:4
        for k=1:4
            predict_senor_covariance(j,k)=predict_senor_covariance(j,k)+(p_data(i,j)-sensor_data(i+1,j))*(p_data(i,k)-sensor_data(i+1,k));
